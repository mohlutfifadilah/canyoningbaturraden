                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-warning">
                        <div class="card-header">
                            <h3 class="card-title text-white">Edit Photo</h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form method="POST" action="{{ route('photo.update', $photo->id) }}"
                            ...
                            <div class="card-body">
                                @php
                                    $package_id = \App\Models\Package::find($photo->id_package);
                                @endphp
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <select
                                                class="custom-select form-control-border @if(session('id_package')) is-invalid @endif @error('id_package') is-invalid @enderror"
                                                id="id_package" name="id_package">
                                                <option selected readonly value="{{ $package_id->id }}">{{ $package_id->name }}</option>
                                                @foreach ($package as $p)
                                                    @if ($p->name != $package_id->name)
